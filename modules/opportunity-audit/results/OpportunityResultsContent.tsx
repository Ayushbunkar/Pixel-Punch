...
export default function OpportunityResultsContent() {
  ...
  const [isUnlocked, setIsUnlocked] = useState(false); // Default to false
  const [emailModalOpen, setEmailModalOpen] = useState(false);
  const [unlockModalOpen, setUnlockModalOpen] = useState(false);


  // Function to handle email submission for unlocking content
  const handleEmailUnlock = useCallback(async (email: string) => {
    try {
      const response = await fetch("/api/opportunity-scan/unlock", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ email, submissionId: data?.submissionId }),
      });
  ...
  const triggerPdfDownload = useCallback(async () => { 
    try {
      const element = document.getElementById("opportunity-pdf-report-content"); // Corrected ID
      if (!element) {
      ...
      const currentData = parsedData; // Renamed to avoid conflict with state variable
      ...
      // Calculate counts
      const recommendationsCount = currentData.recommendations ? currentData.recommendations.length : 0;
      const nextStepsCount = currentData.nextSteps ? currentData.nextSteps.length : 0;
          ...
          <table border="0" cellpadding="0" cellspacing="0" width="100%">
            <tr>
              <td width="48%" valign="middle" style="padding: 16px; background: #fff; border-radius: 12px; border: 1px solid #e2e8f0; text-align: center;">
                ${generatePieChart([
                  { label: "Readiness", value: currentData.scorecard.readiness, color: ragColor(currentData.scorecard.readiness) },
                  { label: "Value", value: currentData.scorecard.value, color: ragColor(currentData.scorecard.value) },
                  { label: "Opportunity", value: currentData.scorecard.opportunity, color: ragColor(currentData.scorecard.opportunity) }
                ])}
                <div style="display: flex; gap: 12px; margin-top: 16px; flex-wrap: wrap; justify-content: center;">
                  <div style="display: flex; align-items: center; gap: 6px; font-size: 10px; color: #64748b;">
                    <span style="width: 10px; height: 10px; background: ${ragColor(currentData.scorecard.readiness)}; border-radius: 2px;"></span>
                    <span>Readiness</span>
                  </div>
                  <div style="display: flex; align-items: center; gap: 6px; font-size: 10px; color: #64748b;">
                    <span style="width: 10px; height: 10px; background: ${ragColor(currentData.scorecard.value)}; border-radius: 2px;"></span>
                    <span>Value</span>
                  </div>
                  <div style="display: flex; align-items: center; gap: 6px; font-size: 10px; color: #64748b;">
                    <span style="width: 10px; height: 10px; background: ${ragColor(currentData.scorecard.opportunity)}; border-radius: 2px;"></span>
              ...
              <td width="48%" valign="top">
                <div style="display: flex; flex-direction: column; gap: 12px;">
                  ${[
                    ["AI Readiness", currentData.scorecard.readiness],
                    ["Business Value", currentData.scorecard.value],
                    ["Automation Opportunity", currentData.scorecard.opportunity]
                  ].map(([label, val]) => `
      ...
      const recommendationsHtml = recommendationsCount > 0 ? `
        <div style="padding: 28px 40px; border-bottom: 1px solid #e2e8f0; page-break-inside: avoid;">
          ...
          <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 16px;">
            ${currentData.recommendations.slice(0, 8).map((rec: string, i: number) => `
      ...
      const auditReportHtml = currentData.auditReport ? `
        <div style="padding: 28px 40px; border-bottom: 1px solid #e2e8f0;">
          ...
          <div style="font-size: 11px; color: #475569; line-height: 1.7; white-space: pre-wrap; background: #f8fafc; border-radius: 8px; padding: 16px; border: 1px solid #e2e8f0;">${cleanMarkdownForPdf(currentData.auditReport)}</div>
        </div>
      ` : "";
      ...
      const htmlContent = `
        <div style="font-family: 'Segoe UI', system-ui, sans-serif; color: #0f172a; background: #fff; padding: 0; margin: 0;">
          ...
          <div style="background: linear-gradient(135deg, #0f172a 0%, #312e81 50%, #4338ca 100%); padding: 36px 40px; margin-bottom: 0;">
            ...
            <p style="color: rgba(255,255,255,0.65); font-size: 12px; margin: 0;">Scan ID: ${currentData.submissionId} &middot; Generated by Pixel Punch AI</p>
          </div>
          ...
          <div style="padding: 20px 40px; background: #0f172a; text-align: center; page-break-inside: avoid;">
            <p style="color: rgba(255,255,255,0.5); font-size: 10px; margin: 0;">Generated by Pixel Punch AI &middot; pixelpunch.org &middot; &copy; 2026 Pixel Punch. Confidential.</p>
          </div>
        </div>
      `;
      ...
      const opt = {
        ...
        filename: `Pixel-Punch-Opportunity-Audit-${currentData.submissionId?.slice(0, 8) ?? "report"}.pdf`,
  ...
  }, [cleanMarkdownForPdf, data]); // Added data to dependencies
  ...
  useEffect(() => {
    if (!loading && data && autoDownload) {
      const timer = setTimeout(() => {
        triggerPdfDownload(); // Removed data.submissionId from call
    ...
    <main className="min-h-screen bg-[#fafbff] pb-12 overflow-x-hidden">
      ...
      >
        ...
        >
          ...
          <p className="text-slate-600 text-sm">
            Customized for <strong className="text-slate-800">{data.company.name}</strong> &middot; {(data.company.businessType ?? data.company.industry ?? "Business").toUpperCase()} ({(data.company.size ?? "unknown").replace("_", "-")} employees)
          </p>
        </motion.div>
              ...
              <div className="relative">
                ...
                {!isUnlocked && (
                  <LockOverlay
                    ...
                    description="Don't fall behind. Secure full insights and unlock the other 5 performance dimensions now."
            ...
            <div className="relative">
              ...
              {/* Lock Overlay */}
                {!isUnlocked && (
                  <LockOverlay
                    onUnlock={() => setEmailModalOpen(true)}
                    title="Premium AI Opportunities Locked"
                    description="Unlock to reveal custom systems generated to address your workflow requirements."
              ...
              {data.nextSteps.length > 2 && (
                <div className="relative group">
                  ...
                  >
                    <LockOverlay
                      onUnlock={() => setEmailModalOpen(true)}
                      title="Premium Steps Locked"
                      description={`Unlock to see all ${data.nextSteps.length} steps`}
      ...
      <div id="opportunity-pdf-report-content" data-json-data={JSON.stringify(data)} style={{ position: "absolute", left: "-9999px", top: 0, width: "794px", backgroundColor: "#fff", padding: "32px", fontFamily: "system-ui, sans-serif", display: "none" }}>
        <div style={{ borderBottom: "2px solid #4f46e5", paddingBottom: "16px", marginBottom: "24px" }}>
          <div style={{ fontSize: "20px", fontWeight: "800", color: "#0f172a" }}>Pixel Punch AI &mdash; Opportunity Audit Report</div>
        ...
        <div style={{ borderTop: "1px solid #e2e8f0", paddingTop: "12px", fontSize: "10px", color: "#94a3b8", textAlign: "center" }}>Generated by Pixel Punch AI &middot; pixelpunch.org &middot; &copy; 2026 Pixel Punch</div>
      </div>
