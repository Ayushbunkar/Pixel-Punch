...
import { Search, CheckCircle2, Cpu, Activity, Lock, Unlock, AlertCircle, Download, Square } from "lucide-react";
    ...
    try {
      ...
      const auditReportHtml = r.auditReport ? `
        <div style="padding: 28px 40px; border-bottom: 1px solid #e2e8f0;">
          <div style="font-size: 11px; color: #475569; line-height: 1.7; white-space: pre-wrap; background: #f8fafc; border-radius: 8px; padding: 16px; border: 1px solid #e2e8f0; text-align: left;">${cleanMarkdownForPdf(r.auditReport)}</div>
        </div>
      ` : "";
      ...
      const htmlContent = `
        <div style="font-family: 'Segoe UI', system-ui, sans-serif; color: #0f172a; background: #fff; padding: 0; margin: 0;">
          <!-- Header -->
          <div style="background: linear-gradient(135deg, #0d6efd 0%, #0b5ed7 100%); padding: 36px 40px; margin-bottom: 0;">
            <div style="display: flex; align-items: center; gap: 10px; margin-bottom: 16px;">
              <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="white" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="color:white;"><rect width="18" height="18" x="3" y="3" rx="2" ry="2"></rect></svg>
              <span style="color: #fff; font-size: 18px; font-weight: 900; letter-spacing: -0.5px;">Pixel Punch</span>
              <span style="color: rgba(255,255,255,0.6); font-size: 11px; letter-spacing: 2px; text-transform: uppercase; font-weight: 600; margin-left: 10px;">AI Cost Audit</span>
            </div>
  ...
  return (
    ...
    <div className="min-h-screen bg-[#fafbff] pb-12 overflow-x-hidden">
      ...
      <div className="bg-blue-600 text-white p-4">
        <div className="max-w-6xl mx-auto flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <span className="text-xl font-bold">Pixel Punch</span>
          </div>
        </div>
      </div>

      {/* AI Cost Audit Title and Date */}
      <div className="max-w-6xl mx-auto px-4 py-8 md:py-10 text-center">
        <h1 className="text-4xl font-bold text-slate-900 mb-2">AI Cost Audit</h1>
        <p className="text-slate-600 text-sm">Generated: 04 July 2026</p>
      </div>

      {/* Submission ID and Company Info */}
      <div className="max-w-6xl mx-auto px-4 mb-8">
        <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-4 text-sm text-slate-700 flex flex-wrap items-center justify-center divide-x divide-slate-200">
          <span className="px-4 py-1"><span className="font-semibold">Submission ID:</span> {result.submissionId}</span>
          <span className="px-4 py-1"><span className="font-semibold">Company:</span> {result.contact?.company || "papapapaa"}</span>
          <span className="px-4 py-1"><span className="font-semibold">Contact:</span> {result.contact?.name || "Ayush Bunkar"}</span>
          <span className="px-4 py-1"><span className="font-semibold">Company Size:</span> {result.contact?.companySize || "small-to-midsize"}</span>
          <span className="px-4 py-1"><span className="font-semibold">Business Type:</span> {result.contact?.businessType || "technology"}</span>
        </div>
      </div>
      ...
      <motion.div 
        ...
        className="max-w-6xl mx-auto px-4 py-8 md:py-10 space-y-4"
      ...
      >
        ...
        {result.confidenceScore && (
          ...
          <motion.div 
            ...
            className="mb-6 bg-white rounded-xl border border-slate-200 p-6 shadow-sm"
          ...
          >
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
              ...
              {/* Confidence Indicator - Single Wide Box */}
              <div className="flex flex-col items-center justify-center p-4 rounded-xl bg-blue-50 border border-blue-200 shadow-sm w-full mx-auto">
                <span className="text-5xl font-black text-blue-700 mb-2">{result.confidenceScore}</span>
                <span className="text-sm uppercase font-bold px-3 py-1 rounded-full bg-blue-100 text-blue-700">
