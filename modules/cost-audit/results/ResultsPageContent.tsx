...
export default function ResultsPageContent() {
  ...
  const triggerPdfDownload = useCallback(async (submId?: string) => {
    ...
    try {
      ...
      const element = document.getElementById("cost-pdf-report-content");

      if (!element) { setDlState("idle"); return; }
          ...
          {dlState === "done" ? (
            <>
              ...
              <div className="flex flex-col gap-2">
                <button onClick={() => triggerPdfDownload(result.submissionId)} className="px-4 py-2.5 bg-white/10 hover:bg-white/15 text-white border border-white/20 rounded-lg text-sm font-medium transition-all">⬇ Download Again</button>
